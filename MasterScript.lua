...
function eventTextAreaCallback(textAreaID, playerName, callback)
  ...
  if textAreaID == 5 then
    if callback == "ask" then
      ...
      ui.addTextArea(4, "<a href='event:spawnselect'><VP>spawn:</VP> " .. itemname(tonumber(callback)) .. "</a>", playerName, 280, -17, 0, 0, 0x000000, 0x222222, 0.3, true)
      mouse[playerName] = "spawn"
...
function eventPopupAnswer(popupID, playerName, answer)
  ...
  if popupID == 9 and tonumber(answer) == nil then
    answer = itemname(string.lower(answer))
    spawntype[playerName] = answer
    ui.updateTextArea(4, "<a href='event:spawnselect'><VP>spawn:</VP> " .. itemname(answer) .. "</a>", playerName)
    mouse[playerName] = "spawn"
    ui.updateTextArea(1, "<a href='event:clicktype'><VP>click type:</VP> spawn</a>", playerName)
  elseif popupID == 9 then
    iname = itemname(tonumber(answer))
    spawntype[playerName] = itemname(tonumber(answer))
    ui.updateTextArea(4, "<a href='event:spawnselect'><VP>spawn:</VP> " .. iname .. "</a>", playerName)
    mouse[playerName] = "spawn"
    ui.updateTextArea(1, "<a href='event:clicktype'><VP>click type:</VP> spawn</a>", playerName)
  end
end